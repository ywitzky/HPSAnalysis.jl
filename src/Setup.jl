            ...
            if ind> (SeqLength-3) 
                ...
                if ! haskey(dihedral_map, key)
                    ...
                    if same_eps_ind===nothing
                        dihedral_cnt+=1
                        dihedral_map[key]=dihedral_cnt
                        dihedral_eps[dihedral_cnt] = round(eps;digits=4)
                    else
                        dihedral_map[key]=same_eps_ind
                    end
                end
            end
        end
...
function writeHPSLammpsScript(fileName, StartFileName, AtomTypes, LongAtomTypes, AaToId, LongAtomTypesToRes, OneToCharge, OneToSigma, OneToLambda, Dihedral_eps, InitStyle, SimulationType, Temperature, AlphaStruct, Restart, NTimeSteps; WriteOutFreq=100_000,SaltConcentration=-1, pH = 6, ChargeTemperSteps=[], ChargeTemperSwapSteps=100_000, OutFormat="h5md")
    ...
    for  ResType in LongAtomTypes
        ...
        for  ResType2 in LongAtomTypes
           ...
           # println(" $id $id2 $ResType $ResType2")
        ...
        for (atom,_) in enumerate(Seq)
            ...
            for i in 1:3
                ...
                if abs(diff[i]-boxSize[i])<abs(diff[i]) # || abs(diff[i]+boxSize[i])<abs(diff[i])
                    #println("diff $(diff[i]), boxSize $(boxSize[i]), $(round(diff[i]/(boxSize[i]/2.))) ")
...
function writeGSDStartFile(FileName::String, NAtoms::I, NBonds::I, NAngles::I, NDihedrals::I,Box::Vector{R}, Positions::Array{R}, AaToId::Dict,Sequences,  InputImage::Array{I}, InputMasses::Array{I}, InputCharges::Arraz{I}) where {I<:Integer, R<:Real}
    ...
    snapshot.particles.position = reshape(Postions, (size(Positions, 1)*size(Positions, 2), 3)) ### reshape to match gsd formart
    snapshot.particles.types =  values(AaToId)
    snapshot.particles.typeid = [Int32(AaToId[AA]) for AA in Seq for Seq in Sequences]
