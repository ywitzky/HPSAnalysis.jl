...
"""
function CreateStartConfiguration(SimulationName::String, Path::String, BoxSize::Vector{ChoosenFloatType}, Proteins::Vector{String}, Sequences::Vector{String} ; Axis="y", Regenerate=true,SimulationType="Calvados2",ProteinToDomain=Dict(),ProteinToCif=Dict(), Data=SimData(), prerun=false)
    if !prerun
        ...
        traj = GSDFormat.open("$(Path)/traj.gsd")
        for frame in traj
            position_subset = frame.particles.position[1:end, 1:end]

            Data.x[:, 1] = position_subset[:, 1]
            Data.y[:, 1] = position_subset[:, 2]
            Data.z[:, 1] = position_subset[:, 3]
        end
    end
